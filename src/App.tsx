...
import type { Data, DataFromJson, ChartDataItem, ExtendedVariation, WeeklyData } from './types/types'
import { ThemeProvider } from './contexts/ThemeContext'
import OptionPanel from './components/OptionPanel/OptionPanel'
import Chart from './components/Chart/Chart'
import './App.css'

const dataFromJson: DataFromJson = {
  "variations": [
    {
      "name": "Original"
    },
...
function App() {
  ...
  const [variationsList, setVariationsList] = useState<ExtendedVariation[]>([]);
  const [isDay, setIsDay] = useState(true);


  useEffect(() => {
    setVariationsList(
      dataFromJson.variations.map(v => ({ ...v, isActive: true }))
    );
  }, []);



  const handleChangeVariations = useCallback((name: string) => {
    setVariationsList(prev => {
      const active = prev.filter(v => v.isActive);
      const target = prev.find(v => v.name === name);

      if (active.length === 1 && target?.isActive) return prev;

      return prev.map(v =>
        v.name === name ? { ...v, isActive: !v.isActive } : v
      );
    });
  }, []);
  ...
  const activeVariations = useMemo(
    () => variationsList.filter(v => v.isActive),
    [variationsList]
  );


  const dailyData = useMemo(() => {
    const data: Data[] = dataFromJson.data;
    ...
    return data.map(item => {
      ...
      activeVariations.forEach(variant => {
        const id = (variant.id ?? 0).toString();
        const visits = item.visits[id] ?? 0;
        const conv = item.conversions[id] ?? 0;
        row[variant.name] = visits ? (conv / visits) * 100 : 0;
      });
  ...
  const weeklyData = useMemo(() => {
    ...
    for (let i = 0; i < dailyData.length; i += 7) {
      ...
      activeVariations.forEach(v => (avg[v.name] = 0));

      chunk.forEach(row => {
        activeVariations.forEach(v => {
          avg[v.name] += (row[v.name] as number);
        });
      });

      activeVariations.forEach(v => {
        avg[v.name] = Number((avg[v.name] / chunk.length).toFixed(2));
      });

      result.push({
        date: `Week ${result.length + 1}`,
        ...avg
      });
    }
  ...
  }, [dailyData, activeVariations]);



  const chartData = isDay ? dailyData : weeklyData;
  ...
  return (
    <ThemeProvider>
      <OptionPanel
        changeVariations={handleChangeVariations}
        changeLine={setLineStyle}
        changeIsDay={setIsDay}
        variations={variationsList}
      />

      <Chart
        data={chartData.map((item, index) => ({ ...item, __index: index }))}
